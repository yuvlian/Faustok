...
mod tiklydown;
use database::*;
use tiklydown::{get_image, get_video, get_music};
...
include!("config.rs"); // Contains constants: SLIDESHOW_CHUNK_VALUE, BOT_PREFIX, and HELP_COMMAND
...
async fn autofix(
    ...
    ctx.say(format!(r#"Autofix for id "{}" has been updated to **{}**"#, user_id, value)).await?;
    Ok(())
}
...
async fn check_autofix(
    ...
    let user_id = user.as_ref().map_or_else(|| ctx.author().id.to_string(), |u| u.id.to_string());
    let settings = ctx.data().user_settings.read().await;
    let value = settings.user_map.get(&user_id).unwrap_or(&false);
    
    ctx.say(format!(r#"Autofix for id "{}" is currently **{}**"#, user_id, value)).await?;
    Ok(())
}
...
async fn vid(
    ...
    let attachment = serenity::CreateAttachment::path(std::path::Path::new(&video_filename)).await?;
...
async fn mp3(
    ...
    let attachment = serenity::CreateAttachment::path(std::path::Path::new(&music_filename)).await?;
    ctx.send(poise::reply::CreateReply {
        attachments: vec![attachment],
        ..Default::default()
    }).await?;
...
async fn event_handler(
    ...
    match event {
        ...
        serenity::FullEvent::Message { new_message } => {
            ...
            // Check for command presence and whether auto-replace is enabled.
            let command = content.contains(".vid") || content.contains(".img") || content.contains(".mp3");
            let should_replace = *settings.user_map.get(&user_id).unwrap_or(&false);

            // Handle link replacement and command suppression.
            if should_replace && TIKTOK_REGEX.is_match(content) && !command {
                ...
                // Edit the original message to suppress embeds and reply with the modified message.
                new_message.clone().edit(ctx, embed_remover).await?;
                new_message.reply(ctx, tiktok_reply).await?;
            } else if command {
                // Suppress embeds for command messages.
                let remove_embed = serenity::MessageFlags::SUPPRESS_EMBEDS;
                let embed_remover = serenity::EditMessage::new().flags(remove_embed);
                new_message.clone().edit(ctx, embed_remover).await?;
            }
